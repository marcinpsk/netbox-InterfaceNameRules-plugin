...
_COMPARISON_INTRO = (
    "Generated by `performance/compare.py` from the `{before}` (before) and `{after}` (after) "
    "artifacts. SQL statement counts are deterministic for equivalent inputs. "
...
def _comparison_intro(before, after):
    """Return the introduction, naming each run by the kind its artifact records."""
    return _COMPARISON_INTRO.format(before=before["baseline_kind"], after=after["baseline_kind"])
...
def main(argv):
    ...
    report = [
        ...
        "",
        _comparison_intro(before, after),
        "",
        "## Environment",
        "",
