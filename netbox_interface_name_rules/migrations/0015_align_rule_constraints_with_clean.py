...
def normalize_invalid_rules(apps, schema_editor):
    ...
    device_rules = rules.filter(applies_to_device_interfaces=True)
    device_rules.filter(module_type_is_regex=True).update(module_type_is_regex=False)
    device_rules.filter(breakout_mode=CHANNELIZED).update(breakout_mode=FLAT)
    rules.filter(breakout_mode=CHANNELIZED, channel_count=0).update(breakout_mode=FLAT)
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.AddConstraint(
            ...
            constraint=models.CheckConstraint(
                condition=(
                    (models.Q(applies_to_device_interfaces=False) | ~models.Q(breakout_mode=CHANNELIZED))
                    & (models.Q(applies_to_device_interfaces=False) | models.Q(parent_name_template=""))
                    & (models.Q(parent_name_template="") | models.Q(breakout_mode=CHANNELIZED))
                    & (~models.Q(breakout_mode=CHANNELIZED) | ~models.Q(channel_count=0))
                ),
