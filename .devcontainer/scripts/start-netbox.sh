...
if [ ! -f "$NETBOX_SETTINGS" ]; then
  echo "⚠️  debug-toolbar USE_SHADOW_DOM patch skipped: $NETBOX_SETTINGS not found"
elif ! grep -q "USE_SHADOW_DOM" "$NETBOX_SETTINGS"; then
  ...
  elif sed -i "/'SHOW_TOOLBAR_CALLBACK': 'utilities.debug.show_toolbar',/a\\    'USE_SHADOW_DOM': False," "$NETBOX_SETTINGS"; then
    echo "🔧 Patched debug-toolbar USE_SHADOW_DOM=False in settings.py"
  else
    echo "⚠️  debug-toolbar USE_SHADOW_DOM patch failed: could not write $NETBOX_SETTINGS" >&2
  fi
fi
...
if [ ! -f "$NETBOX_LOCAL_SETTINGS" ]; then
  echo "⚠️  debug-toolbar ProfilingPanel patch skipped: $NETBOX_LOCAL_SETTINGS not found"
elif ! grep -q "DEBUG_TOOLBAR_PANELS" "$NETBOX_LOCAL_SETTINGS"; then
  if cat >> "$NETBOX_LOCAL_SETTINGS" <<'PYEOF'
...
PYEOF
  then
    ...
    echo "⚠️  debug-toolbar ProfilingPanel patch failed: could not write $NETBOX_LOCAL_SETTINGS" >&2
  fi
fi
