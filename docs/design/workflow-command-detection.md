...
The detector recognizes literal `pytest` commands, versioned Python 3 module
invocations, and the `poetry run`, `uv run`, and `xvfb-run` prefixes. It skips
the supported `xvfb-run` options before it inspects the wrapped command. It
inspects command substitutions separately. GitHub expressions become opaque
shell expansions before parsing. Variable values, ANSI-C quoting, arbitrary
wrappers, aliases, `eval`, and programs embedded in `bash -c` are outside this
static check.
